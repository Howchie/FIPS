...
.FJK_derivs <- function(t, state, parms, df, dynamic_vars = NULL) {
...
FJK_simulation_dispatch  <- function(dat, pvec, model_formula = NULL, 
                                     ...
                                     method="lsoda") {
  dynamic_vars <- fips_normalize_dynamic_vars(dynamic_vars, model = "FJK")
  pvec <- .fips_adjust_pvec_for_light_metric(pvec, dynamic_vars, model_name = "FJK")
  FJK_check_pvec(pvec)
  ...
  ODE_results = as.data.frame(deSolve::ode(
    ...
    times = dat$t_abs,
    func = .FJK_derivs,
    parms = pvec, method = method, df = dat, dynamic_vars = dynamic_vars, rootfun = .FJK_rootfun,
