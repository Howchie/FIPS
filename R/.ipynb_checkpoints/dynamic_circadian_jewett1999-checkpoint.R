...
.K99_derivs <- function(t, state, parms,df) {
...
K99_simulation_dispatch  <- function(dat, pvec, model_formula = NULL, 
                                     dynamic_vars=list(approx_N=FALSE,model="K99",light_levels=lux_levels()), 
                                     method="lsoda") {
  K99_check_pvec(pvec)
  ...
  ODE_results = as.data.frame(deSolve::ode(
    ...
    times = dat$t_abs,
    func = .K99_derivs,
    parms = pvec, method = method, df = dat, rootfun = .K99_rootfun,
   ...
   dat <- dat %>%
       ...
       dplyr::renamerename(wake_status=out_of_bed)
